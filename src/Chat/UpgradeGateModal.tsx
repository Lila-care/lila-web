...
export default function UpgradeGateModal({ onClose, onSubscribed }: UpgradeGateModalProps) {
  const { token } = useAuth()
  ...
  const handleUpgrade = async () => {
    ...
    const paymentSourceId = await openCheckout({
      amountInCents: PLAN_AMOUNT_IN_CENTS,
      reference: `lila-sub-${Date.now()}`,
    })
    // User closed the widget, or it failed to open — `checkoutError` already surfaces why.
    if (!paymentSourceId) return

    setIsSubmitting(true)
    try {
      await createCheckout(token, { paymentSourceId, planId })
      onSubscribed?.()
      onClose()
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'No se pudo procesar el pago')
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {busy ? 'Procesando...' : `Actualizar a Premium — ${PLAN_PRICE_LABEL}`}
        </button>
