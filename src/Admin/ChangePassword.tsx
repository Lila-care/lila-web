...
function ChangePassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !session) {
      setError('Sesión inválida. Regresa al inicio de sesión e intenta de nuevo.')
      return
    }
    ...
    <div className="w-full min-h-screen flex items-center justify-center bg-secondary">
      <div className="bg-white shadow-lg rounded-2xl p-10 w-full max-w-md text-center space-y-6">
        ...
        <p className="text-gray-500 text-sm">
          Esta es la primera vez que accedes. Debes establecer una contraseña nueva.
        </p>

        <form onSubmit={handleSubmit} className="space-y-4 text-left" data-testid="change-password-form">
          <div>
          ...
          {error && (
            <p className="text-red-600 text-sm" data-testid="change-password-error">
              {error}
            </p>
          )}
